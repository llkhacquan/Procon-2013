...
namespace Procon.Core
{
    ...
    public class Symbol
    {
        ...
        public static int getIndex(char c)
        {
            ...
            try
            {
                index = C.DEFAULT_SYMBOLS_SET.IndexOfAny(c1);
            }
            catch (Exception)
            {
                index = -1;
            ...
            if (index == -1)
                throw new Exception(String.Format("Input symbol is not in the symbols set: '{0}'", c));
            return index;
        }
        ...
        public static char getCharacter(int i)
        {
            ...
            if (i > C.DEFAULT_SYMBOLS_SET.Length - 1)
            {
                throw new Exception("i>85");
            }
            return C.DEFAULT_SYMBOLS_SET[i];
        }
    }
}
